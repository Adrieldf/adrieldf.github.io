...
function ProjectImage({ project, theme }: { project: Project, theme: Theme }) {
...
export default function Home() {
  ...
  // Default theme alternates by day; computed client-side only so the static export stays hydration-safe.
  const dayTheme = useSyncExternalStore<Theme>(
    () => () => { },
    () => (new Date().getDate() % 2 === 0 ? "retro" : "cyberpunk"),
    () => "retro",
  );
  const [themeOverride, setThemeOverride] = useState<Theme | null>(null);
  const theme = themeOverride ?? dayTheme;
  ...
  const toggleTheme = () => {
    setThemeOverride(theme === "retro" ? "cyberpunk" : "retro");
    setIsCRT(false);
    setIsGlitch(false);
  };
